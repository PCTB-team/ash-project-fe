    ...
    <Modal open={open} onCancel={handleCancel} footer={null} width={480} centered destroyOnHidden
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, type: "spring", stiffness: 300, damping: 25 }}>
        <div className="bg-gradient-to-br from-[var(--color-primary)] to-[#1a1a1a] px-8 py-8 flex flex-col items-center relative overflow-hidden text-center">
          ...
          <div className="w-16 h-16 rounded-[20px] bg-white/15 border border-white/25 flex items-center justify-center text-white backdrop-blur-md shadow-xl relative z-10 mb-4">
            <i className="bi bi-cloud-arrow-up-fill text-[32px]" />
          </div>
          <h3 className="text-[20px] font-semibold text-white tracking-tight leading-tight relative z-10">Upload Tài liệu</h3>
        ...
        <div className="p-8 bg-[var(--color-surface)]">
          <Upload.Dragger {...uploadProps}
            className="!rounded-[24px] !border-2 !border-dashed !border-[var(--color-primary)]/25 hover:!border-[var(--color-primary)] !bg-[var(--color-surface-container-low)] hover:!bg-[var(--color-primary)]/5 mb-6 transition-all duration-300 py-10 group overflow-hidden relative shadow-none hover:shadow-inner">
          ...
          {file && (
            <div className="mb-6 p-4 rounded-[16px] bg-[var(--color-surface-container-low)] border border-black/[0.04] flex items-center gap-3">
          ...
          <div className="flex gap-3">
            ...
            <Button type="primary" onClick={handleUpload} loading={uploading} disabled={!file}
              className="flex-1 !rounded-xl !font-medium !text-[13px] !h-12 !shadow-lg !shadow-[var(--color-primary)]/20 !border-none !bg-[var(--color-primary)]">
